...
class GaussianDiffusionModelCast(nn.Module):
    ...
    def forward(self,  snapshots: torch.Tensor, conditioning_snapshots: torch.Tensor, 
                previous_snapshots: torch.Tensor, s: torch.Tensor, Reynolds_number: torch.Tensor) -> torch.Tensor:
        ...
        # # ===== testing ===
        # alpha = 0.1
        # lam = np.random.beta(alpha+1, alpha) if alpha > 0.0 else 1.0
        # index = torch.randperm(snapshots.size()[0]).cuda()
        # previous_snapshots = lam * previous_snapshots + (1. - lam) * snapshots        
        # conditioning_snapshots_interpolated = lam * conditioning_snapshots_interpolated + (1. - lam) * snapshots       
        # # ===== testing ===


        residual_snapshots = snapshots - conditioning_snapshots_interpolated - previous_snapshots
        ...
        residual_snapshots_t = alpha * residual_snapshots + sigma * eps
        ...
        eps_predicted = self.eps_model(residual_snapshots_t, _ts, 
                                       lowres_snapshots=conditioning_snapshots_interpolated,
                                       previous_snapshots=previous_snapshots, s=s, Re=Reynolds_number)
    ...
    def sample(self, n_sample: int, size, conditioning_snapshots: torch.Tensor,
               previous_snapshots: torch.Tensor, s: torch.Tensor, Reynolds_number: torch.Tensor, 
        ...
        if self.sampler == 'ddpm':  
            ...
            for time_step in range(self.n_T, 0, -1):
                ...
                pred = self.eps_model(snapshots_i, time.to(device), lowres_snapshots=conditioning_snapshots_interpolated,
                                      previous_snapshots=previous_snapshots, s=s, Re=Reynolds_number)
        ...
        return snapshots_i + conditioning_snapshots_interpolated + previous_snapshots
