...
from src.get_data import NSTK_train as NSTK
...
class Trainer:
    ...
    def _run_batch(self, targets, conditioning_lr_snapshots, previous_snapshots, s, Reynolds_number):
        self.optimizer.zero_grad()
        loss = self.model(targets, conditioning_lr_snapshots, previous_snapshots, s, Reynolds_number)
        loss.backward()
        self.optimizer.step()
    ...
    def _run_epoch(self, epoch):
        ...
        loss_values = []
        for conditioning_lr_snapshots, previous_snapshots, targets, s, Reynolds_number in self.train_data:
            conditioning_lr_snapshots = conditioning_lr_snapshots.to(self.gpu_id)
            previous_snapshots = previous_snapshots.to(self.gpu_id)
            s = s.to(self.gpu_id)
            Reynolds_number = Reynolds_number.to(self.gpu_id)

            targets = targets.to(self.gpu_id)            
            loss_values.append(self._run_batch(targets, conditioning_lr_snapshots, previous_snapshots, s, Reynolds_number))
        return loss_values
    ...
    def _generate_samples(self, epoch):
        with self.model.module.ema.average_parameters():
            ...
            with torch.no_grad():
                ...
                conditioning_lr_snapshots, previous_snapshots, targets, s, Reynolds_number = next(iter(self.train_data))
                conditioning_lr_snapshots = conditioning_lr_snapshots.to('cuda')
                previous_snapshots = previous_snapshots.to('cuda')
                s = s.to('cuda')
                ...
                samples = self.model.module.sample(targets.shape[0], (1, targets.shape[2], targets.shape[3]), 
                                               conditioning_lr_snapshots, previous_snapshots, s, Reynolds_number,
                                               'cuda')
    ...
    def train(self, max_epochs: int):
        ...
        self.lr_scheduler = scheduler(
            ...
            num_warmup_steps=len(self.train_data) * 1, #5, # we need only a very shot warmup phase for our data
...
def main(rank: int, world_size: int, sampling_freq: int, epochs: int, batch_size: int, run, args):
    ...
    dataset, model, optimizer = load_train_objs(args=args)
...
if __name__ == "__main__":
    ...
    # Forecasting and super-res parameters
...
# export CUDA_VISIBLE_DEVICES=0,1,2,3;  python trainFC_nstk.py  --factor 8 --batch-size 20 --run-name run_FC_ddim10_unet64_v8 --sampler ddim --time-steps 10 --prediction-type v --num-pred-steps 3 --base-width 64 