...
class Trainer:
    ...
    def _run_batch(self, targets_SR,targets_FC, conditioning_lr_snapshots, past_snapshots, s, Reynolds_number):
        self.optimizer.zero_grad()
        loss = self.model(targets_SR,targets_FC, conditioning_lr_snapshots, past_snapshots, s, Reynolds_number)
        loss.backward()
        self.optimizer.step()
    ...
    def _run_epoch(self, epoch):
        ...
        loss_values = []
        for conditioning_lr_snapshots, past_snapshots, targets_SR,targets_FC, s, Reynolds_number in self.train_data:
            conditioning_lr_snapshots = conditioning_lr_snapshots.to(self.local_gpu_id)
            past_snapshots = past_snapshots.to(self.local_gpu_id)
            s = s.to(self.local_gpu_id)
            Reynolds_number = Reynolds_number.to(self.local_gpu_id)

            targets_SR = targets_SR.to(self.local_gpu_id)
            targets_FC = targets_FC.to(self.local_gpu_id)
            loss_values.append(self._run_batch(targets_SR,targets_FC,
                                    conditioning_lr_snapshots, past_snapshots, s, Reynolds_number))
        return loss_values
    ...
    def _generate_samples(self, epoch):
        with self.ema.average_parameters():
            ...
            with torch.no_grad():
                ...
                conditioning_lr_snapshots, past_snapshots, targets_SR,targets_FC, s, Reynolds_number = next(iter(self.train_data))
                conditioning_lr_snapshots = conditioning_lr_snapshots.to('cuda')
                s = s.to('cuda')
                Reynolds_number = Reynolds_number.to('cuda')

            
                samples = self.model.module.sample(targets_SR.shape[0],
                                                   (1, targets_SR.shape[2], targets_SR.shape[3]), 
                                                   conditioning_lr_snapshots, s, Reynolds_number,'cuda')
                     
            
        plot_samples(samples, conditioning_lr_snapshots, targets_SR, PATH, epoch)
...
def load_train_objs(superres, args):
    ...
    unet_model,lowres_head,past_head = UNet(image_size=256, in_channels=1, out_channels=1, 
    ...
    model = GaussianDiffusionModelCast(eps_model=unet_model.cuda(),
                                       lowres_model = lowres_head.cuda(),
                                       past_model = past_head.cuda(),
                                       betas=(1e-4, 0.02),
                                       n_T=args.time_steps, 
